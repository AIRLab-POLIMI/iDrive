...
class MongoPlayback(object):
    ...
    def setup(self, database_name, req_topics, user_start_time, user_end_time):
	...
	if user_start_time == "null" or user_end_time == "null":		
	    ...
	    start_time = to_ros_time(datetime.datetime.strptime(user_start_time, '%Y-%m-%d_%H:%M:%S.%f'))
	    end_time =  to_ros_time(datetime.datetime.strptime(user_end_time, '%Y-%m-%d_%H:%M:%S.%f'))
...
def main(argv):
    ...
    parser.add_option("--start", dest="start_time",
		      help ="start time for the replay in format YYYY-mm-dd_HH:MM:SS.f",
		      metavar="DATE", default="null")
    parser.add_option("--end", dest="end_time",
		      help ="end time for the replay in format YYYY-mm-dd_HH:MM:SS.f",
		      metavar="DATE", default="null")
    ...
    playback.setup(database_name, topics, start_time, end_time)
    playback.start()    
    playback.join()
    rospy.set_param('use_sim_time', False)
